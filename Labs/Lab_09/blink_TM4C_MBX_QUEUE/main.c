...
//	msgp = &msg;												// init message pointer to address of msg
	...
	while(1){
		...
		Semaphore_pend(???, BIOS_WAIT_FOREVER);					// wait on semaphore from Timer ISR
...
// MAILBOX CODE follows...
//------------------------------
//		Mailbox_pend(???, &msg, BIOS_WAIT_FOREVER);									// wait/block until post of msg, get msg.val
...
void Timer_ISR(void)
{
	...
	Semaphore_post(???);									// post Sem to unblock mailbox-queue-task
